...
export default function MatchesPage() {
  const { items, matches, addMatch, currentUser } = useStore();
  ...
  const runAIScan = async () => {
    ...
    for (const lost of lostItems) {
      for (const found of foundItems) {
        // Simple optimization: only check same category
        if (lost.CategoryID !== found.CategoryID) continue;
        
        // Don't re-match if already exists
        if (matches.some(m => m.LostItem === lost.ItemID && m.FoundItem === found.FoundItem)) continue;
        ...
        try {
          ...
          if (result.isPotentialMatch && result.similarityScore > 0.4) {
            addMatch(lost.ItemID, found.ItemID, result.similarityScore);
            matchCount++;
          }
        } catch (error) {
                  ...
                  <CardFooter className="bg-secondary/30 px-6 py-3 flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" className="gap-1.5">
                        <CheckCircle2 className="h-4 w-4" /> Notify User
                      </Button>
                    </div>
                  </CardFooter>
                </Card>
